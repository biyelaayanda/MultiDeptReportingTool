...
namespace MultiDeptReportingTool.Services
{
    public class MfaService : IMfaService
    {
        ...
        public async Task<bool> DisableMfaAsync(int userId, string currentPassword, string mfaCode)
        {
            try
            {
                var user = await _context.Users.FindAsync(userId);
                ...
                bool mfaValid = await VerifyTotpCodeAsync(userId, mfaCode);
                if (!mfaValid)
                    return false;
        ...
        public async Task<MfaStatusDto> GetMfaStatusAsync(int userId)
        {
            try
            {
                ...
                return new MfaStatusDto
                {
                    ...
                    IsCurrentlyRequired = await IsMfaRequiredAsync(userId),
                    LastUsed = null // Could track this with additional fields
                };
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> IsMfaRequiredAsync(int userId)
        {
            try
            {
                var user = await _context.Users.FindAsync(userId);
            ...
            {
                _logger.LogError(ex, "Error checking MFA requirement for user {UserId}", userId);
                return false;
            }
        }

        public async Task RecordFailedMfaAttemptAsync(int userId)
        {
            try
            {
                var user = await _context.Users.FindAsync(userId);
                ...
                await _auditService.LogSecurityEventAsync(
                    action: "MFA_FAILED_ATTEMPT_RECORDED",
                    resource: "User",
                    userId: userId,
                    username: user.Username,
                    isSuccess: false,
                    failureReason: "Failed MFA attempt",
                    details: $"Failed MFA attempt recorded for user {user.Username}. Total attempts: {user.MfaFailedAttempts}",
                    ipAddress: "Unknown"
                );
            }
            catch (Exception ex)
        ...
        public async Task<bool> IsUserMfaLockedAsync(int userId)
        {
            try
            {
                var user = await _context.Users.FindAsync(userId);
                ...
                return user.MfaLockedUntil.HasValue && user.MfaLockedUntil > DateTime.UtcNow;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error checking MFA lockout status for user {UserId}", userId);
                return false;
            }
        }

        public async Task ResetMfaLockoutAsync(int userId)
        {
            try
            {
                var user = await _context.Users.FindAsync(userId);
                ...
                await _auditService.LogSecurityEventAsync(
                    action: "MFA_LOCKOUT_RESET",
                    resource: "User",
                    userId: userId,
                    username: user.Username,
                    details: $"MFA lockout reset for user {user.Username}",
                    ipAddress: "Unknown"
                );
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error resetting MFA lockout for user {UserId}", userId);
            }
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    await _auditService.LogSecurityEventAsync(
                        ...
                        details: $"MFA verification failed for user {user.Username}. Failed attempts: {user.MfaFailedAttempts}",
                        ipAddress: "Unknown"
                    );
                }
